...
# Function to load a model, inputs hyperparameters and the model name and outputs:
# A tuple with the model and tokenizer.
def load_model(
        model_name: str, 
        max_seq_length: int, 
        load_in_4bit: bool, 
    ...
    # Set upt the peft model based on QLora
...
# Function to finetune the model for both SFT and DPO, 
...
def finetune(
    ...
    if finetuning_type == "sft":
        ...
        # For Simple Fine-Tuning based on instruction dataset.
        def format_samples_sft(examples):
            # Setting up an empty list to store the instructions and outputs.
            text = []

            # Loop throught the instructions and outputs, format the message in alpaca format and eppend to the text.
            for instruction, output in zip(examples["instruction"], examples["output"], strict=False):
                message = alpaca_template.format(instruction, output) + EOS_TOKEN
                text.append(message) # Add the message format.

                return {"text": text}
            
            dataset1 = load_dataset(f"{dataset_huggingface_workspace}/llmtwin", split="train") # My huggingface instruction dataset.
            dataset2 = load_dataset("mlabonne/FineTome-Alpaca-100k") # Maximmes split version of a secondary dataset to refine chat_template learning.
            ...
            print(f"Loaded datasets with {len(dataset)} samples.")
    ...
    # Switching to DPO.        
        ...
        # Function to format the samples from the preference dataset.
        def format_samples_dpo(example):
            example["prompt"] = alpaca_template.format(example["prompt"], "") # Formatting the preference DS based on Alpaca.
        ...
        trainer = DPOTrainer(
            ...
            train_dataset=dataset["train"], 
            eval_dataset=dataset["test"],
            max_length=max_seq_length // 2, # When using DPO divide by two so that both sequences can fit into the input for comparison.
...
def inference(
    ...
    # Initialize the text streamer for generating text
...
def save_model(model: Any, tokenizer: Any, output_dir: str, push_to_hub: bool = False, repo_id: Optional[str] = None):
    ...
    # Optionally push the model to the Huggingface hub
    if push_to_hub and repo_id:
        print(f"Saving model to '{repo_id}'")  # noqa
        model.push_to_hub_merged(repo_id, tokenizer, save_method="merged_16bit")


def check_if_huggingface_model_exists(model_id: str, default_value: str = "sflyranger17/TwinLlama-3.1-8B") -> str:
    api = HfApi()
...
if __name__ == "__main__":
    ...
    # Add argument for batch size per device
    parser.add_argument("--per_device_train_batch_size", type=int, default=2)
    
    # Add argument for learning rate
    parser.add_argument("--learning_rate", type=float, default=3e-4)
    
    # Add argument for Hugging Face workspace for datasets
    parser.add_argument("--dataset_huggingface_workspace", type=str, default="sflyranger17")
    
    # Add argument for Hugging Face workspace for model output
    parser.add_argument("--model_output_huggingface_workspace", type=str, default="sflyranger17")
    
    # Add argument for dummy mode flag
    parser.add_argument("--is_dummy", type=bool, default=False, help="Flag to reduce the dataset size for testing")
    
    # Add argument for finetuning type
    parser.add_argument(
        "--finetuning_type",
        type=str,
        choices=["sft", "dpo"],
    ...
    # Add argument for output data directory
    parser.add_argument("--output_data_dir", type=str, default=os.environ["SM_OUTPUT_DATA_DIR"])
    
    # Add argument for model directory
    parser.add_argument("--model_dir", type=str, default=os.environ["SM_MODEL_DIR"])
    
    # Add argument for number of GPUs
