...
class SagemakerHuggingfaceStrategy(DeploymentStrategy):
    def __init__(self, deployment_service) -> None:
        """
        Initializes the deployment strategy with the neccessary services for Huggingface.
    ...
    # Implementing the abstract method to deploy
    def deploy(
            self, 
            role_arn: str, 
            llm_image: str, 
        ...
        logger.info(
            f"Deployment parameters: number of replicas: {settings.COPIES}, number of gpus: {settings.GPUS}, instance_type: {settings.GPU_INSTANCE_TYPE}"
        )

        try:
            # Delegate the deployment service to handle the actual deployment details.
...
class DeploymentService:
    def __init__(self, resource_manager):
        """
        Initializes the DeploymentService with the necessary dependencies.
        ...
        self.sagemaker_client = boto3.client(
            ...
            aws_secret_key_id=settings.AWS_SECRET_KEY,
        )
    ...
    def deploy(
        ...
        configurations and endpoints as neessary.
        ...
        :param config: The configuration dictionary for the environment variables of the model.
        :param endpoint_name: The name for the Sagemaker endpoint configuration.
        :param resources: Optional resources for the model deployment (used for multi model endpoints)
        :param endpoint_type: can be EndpointType.MODEL_BASED (without inference component)
                or EndpointType.INFERENCE_COMPONENT (with inference component)
